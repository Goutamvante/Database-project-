...
x=PrettyTable()
window=Tk()
window.title('Faculty Details')
window.geometry('650x200')

sr=Label(window,width=20,text='Enter Faculty ID',bg='yellow')
...
def clicked():
    ...
    ch=pd.read_sql('SELECT * FROM faculty_details WHERE fac_ID={}'.format(t1),con=db_connection)
